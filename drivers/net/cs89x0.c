...
/* Check for a network adaptor of this type, and return '0' iff one exists.
   ...
   Return 0 on success.
 * 
 * 在I/O端口上搜索CS89x0网卡,首先确定CS89x0网卡的I/O基地址，
 * 然后调用netcard_probe1来检测网络设备，以确定CS89x0网卡的存在。
 */
...
struct net_device * __init cs89x0_probe(int unit)
{
	...
	/*
	 * 检查引导设置,得到CS89x0网卡的I/O基地址、设备名字等信息。
	 */
	...
	if (io > 0x1ff)	{	/* Check a single specified location. */
	    ...
	    /*
	     * 如果基地址为0,,则检测netcard_portlist中的地址，
	     * 以确定CS89x0网卡是否存在。
	     */
...
cs89x0_probe1(struct net_device *dev, int ioaddr, int modular)
{
	...
	/*
	 * 通过检查芯片型号和修订版号来设置正确的发送命令
	 * CS89x0的C版和F版可采用快速发送命令
	 */
...
/* 获取DMA设置 */
	...
	dev->watchdog_timeo	= HZ; /* 看门狗时钟时间间隔 */
...
/* And 2.3.47 had this: */
	...
	writereg(dev, PP_BusCTL, ENABLE_IRQ
        ...
        //标记__LINK_STATE_XOFF，准备接受上层协议模块发来的数据传输请求
...
static irqreturn_t net_interrupt(int irq, void *dev_id)
{
	...
	while ((status = readword(dev->base_addr, ISQ_PORT))) {
		...
		case ISQ_RECEIVER_EVENT: /* 网卡收到数据包 */
			/* Got a packet(s). */
			net_rx(dev);
			break;
		case ISQ_TRANSMITTER_EVENT: /* 发送成功中断 */
			lp->stats.tx_packets++;
			...
			break;
		case ISQ_BUFFER_EVENT: // 缓冲区准备好，有足够空间容纳待发送的数据包
...
net_rx(struct net_device *dev)
{
    ...
    /* 获取对应的上层协议类型(802.3, 802.2)和设置包的类型(BORADCAST, MULTICAST,OTHERHOST) */
    skb->protocol=eth_type_trans(skb,dev);
