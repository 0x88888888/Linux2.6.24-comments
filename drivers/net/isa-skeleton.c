...
static int __init do_netcard_probe(struct net_device *dev)
{
    ...
    /* 如果基地址被指定，直接检测该地址上的设备，以确保适配器是否存在 */
   ...
   /*
    * 如果基地址为0，则检测netcard_portlist限定的地址，
    * 确定是否有匹配的适配器存在。
    */
...
static void net_tx_timeout(struct net_device *dev)
{
	 ...
	 * of here.
	 *
	 * 如果发送队列有空闲空间容纳数据包,则唤醒被阻塞的发送过程(即NET_TX_SOFTIRQ软中断)
	 */
	if (!tx_full(dev))
 ...
 *
 * net_device->hard_start_xmit函数指针
 * 上层协议通过驱动程序的net_device->hard_start_xmit函数指针发送数据
 */
static int net_send_packet(struct sk_buff *skb, struct net_device *dev)
{
	struct net_local *np = netdev_priv(dev);
...
/*
 * 较新的网络设备工作方式:网络设备通过环形缓冲区存储发送数据包，
 * 支持一次发送多个数据包
 */
#if TX_RING
	 ...
	 *
	 * 当环形缓冲区已经没有多余的空间时，需要暂停设备的发送过程
	 * 有空间腾出后，会触发中断来唤醒设备，继续发送过程
	 */
	if (tx_full(dev))
...
void net_tx(struct net_device *dev)
{
	...
	entry = np->tx_old;
	/*
	 * 循环判断哪些数据包已经被发送，知道所有的包被发送出去
	 * 如果数据已经发送，则删除环形缓冲区中存储该数据的单元
	 */
	while (tx_entry_is_sent(np, entry)) {
		struct sk_buff *skb = np->skbs[entry];

		np->stats.tx_bytes += skb->len;
		dev_kfree_skb_irq (skb); /* 删除环形缓冲区中的数据单元 */
        /* 下一个 */
	 ...
	 * wake up the queue.
	 *
	 * 当环形缓冲区中没有多余空间添加新的套接字缓冲区数据时，即缓冲区满，
	 * 将暂停向网路设备缓冲区写入新的数据包内容
	 * 当环形缓冲区中有空闲的单元后，需要唤醒被暂停的过程，
	 * 允许向缓冲区添加新的数据.
	 */
...
static irqreturn_t net_interrupt(int irq, void *dev_id)
{
	...
	/* 从状态寄存器读取状态值:数据包是否到达、发送是否完成或发生错误 */
	status = inw(ioaddr + 0);
    ...
    // 如果是数据包到达，则通过net_rx接受数据包
	if (status & RX_INTR) {
		/* Got a packet(s). */
		net_rx(dev);
	}
#if TX_RING
    // 如果是数据包发送完成，且采用环形环形缓冲区，则net_tx通知数据包发送结束
	if (status & TX_INTR) {
		/* Transmit complete. */
		net_tx(dev);
		np->stats.tx_packets++;
...
net_rx(struct net_device *dev)
{
	...
	do {
		...
		} else { /* 如果数据包正确到达，则分配一个套接字缓冲区来存储数据包内容 */
