...
struct ppp {
	...
	int		mru;		/* max receive unit 60,最大接受单元 */
 ...
 *  ppp_xmit_process()
 *   ppp_push()
 * 得到用于发送的channel对象，然后把准备好的数据包发送出去。
 */
static void
ppp_push(struct ppp *ppp)
{
	...
	if ((ppp->flags & SC_MULTILINK) == 0) {
		...
		/*
		 * 获得channel发送函数，把数据包交到下层通道，执行发送操作
		 *
		 * 如果ppp下层通道采用PPPoE设备,用pppoe_chan_ops变量,start_xmit函数指向pppoe_xmit.
		 * 当下层通道为异步ppp设备是，用async_ops变量。
		 */
		if (pch->chan) {
