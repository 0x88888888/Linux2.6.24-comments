...
int bus_register(struct bus_type * bus)
{
    ...
    /*
     * 调用add_probe_files接口，
     * 在bus下添加drivers_probe和drivers_autoprobe两个attribute（如/sys/bus/spi/drivers_probe和/sys/bus/spi/drivers_autoprobe），
     * 其中drivers_probe允许用户空间程序主动出发指定bus下的device_driver的probe动作，
     * 而drivers_autoprobe控制是否在device或device_driver添加到内核时，自动执行probe
	 */
	bus->drivers_autoprobe = 1;
    ...
    /*
     * 调用bus_add_attrs，添加由bus_attrs指针定义的bus的默认attribute，
     * 这些attributes最终会体现在/sys/bus/xxx目录下
	 */
