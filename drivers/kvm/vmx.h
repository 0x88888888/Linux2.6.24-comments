...
enum vmcs_field {
    ...
    //下面4个，每位对应一个io port addr, 为一时访问该port产生vm-exit
	...
	//处理器的指令是否引起VM-Exit, bit31控制是否启用SECONDARY_VM_EXEC_CONTROL
	 ...
	 * 属于Host: Guest读该位返回shadow的值,
	 * 写该位时如果与shadow值不等，产生vm-exit.
	...
	/*
	 * 记录退出 VM Exit原因的明细信息,记录退出原因的明细信息
	 *
	 * a)  指令引发的:invplg 保存操作数线性地址， mwait 则为0或1，其它指令记录内存操作数偏移
       ...
       k)  eptviolation引发
           bit0 read, bit1 write, bit2 execute
           bit3 readable, bit4 wirtable, bit5 executable
...
//当guest os访问cr3的时候，出现这个异常，处理函数为handle_cr
