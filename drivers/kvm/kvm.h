...
struct kvm_mmu {
	...
	/*
	 * paging32_page_fault, paging64_page_fault
	 *
	 * 在paging32_init_context中赋值，
	 *
	 * 在paging_tmpl.h中定义
	...
	/*
	 * paging32_gva_to_gpa, paging64_gva_to_gpa
	 */
...
struct kvm_vcpu {
	...
	/*
	 * 在 guest os 退出到host os执行时，保存 guest os的寄存器信息
	 * 在函数vmx_vcpu_run中保存到regs中来
	 */
...
/*
 * 每个slot 管理以 base_gfn 为开始的 guest page number ，的连续npages个page
 */
...
struct kvm {
     ...
     * 映射的内存页数目以及起始宿主机虚拟地址。于是 KVM 就可以实现对客户机物理地址到宿主机虚拟地址之间的转换，
...
static inline int kvm_mmu_page_fault(struct kvm_vcpu *vcpu, gva_t gva,
				     u32 error_code)
{
    //paging32_page_fault  或者paging64_page_fault，在paging_tmpl.h中
