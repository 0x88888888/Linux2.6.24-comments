...
struct in_device
{
	...
	atomic_t		refcnt;
	/* 1表示ip本配置块将要被释放，不在允许其他成员访问了。 */
	int			dead;
	/* 一个net_device可以有多个ip地址 */
	...
	// 记录所属接口最近离开组播的信息，用于报告“组播源列表改变到不希望接受记录”类型的组报告.当所属接口加入到mc_tomb上的组播是，该组播会从这里删除
...
struct in_ifaddr
{
	...
	struct rcu_head		rcu_head;
	/*
	 * ifa_local和ifa_address都是用来存储ip地址的,在不同类型的设备，意义不同
	 * 1.在配置了支持广播的设备上，ifa_local和ifa_address都是指本地ip地址
