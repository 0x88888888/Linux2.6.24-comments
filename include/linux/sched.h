...
#define TASK_TRACED		8    /*调试使用状态，比如gdb attach时*/
/* in tsk->exit_state */
...
struct signal_struct {
    ...
    /*
     * 在do_wait中，父进程等待在wait_chldexit上，
     * 在__wake_up_parent中，唤醒等待在wait_chldexit上的父进程
	 */
 ...
 */
struct sched_entity {
	struct load_weight	load;		/* 当前进程的权重，决定entity占就绪队列总负荷的比例，for load-balancing */
	struct rb_node		run_node;   /* 标准的树节点，红黑树上排序 */
	unsigned int		on_rq;      /* 1:表示已经注册到就绪队列,否则为0 */

    /* 进程开始执行的实际时间 */
	u64			exec_start;
	u64			sum_exec_runtime;   /* 进程总的实际消耗掉的cpu时间 */
	
	u64			vruntime; /* 进程总共执行的虚拟时间 */
	u64			prev_sum_exec_runtime; /* 进程前一次投入运行的总实际时间 */
...
struct task_struct {
	...
	int lock_depth;		/* BKL lock depth,允许多次请求大内核锁(kernel_sem)，值表示请求kernel_sem的次数 */
	...
	unsigned did_exec:1;
	/* 进程的session id 和pgrp在signal成员当中,
	  下面的两个pid是全局id，是属于init_pid_ns的,
	  另外,task_struct->signal->__sessionid, task_struct->signal->__pgid也属于全局命名空间
	  通过系统调用sys_getpid(kernle/timer.c中)
	 */
	pid_t pid;  /* 线程id */
	pid_t tgid; /* 进程id */
	...
	struct task_struct *group_leader;	/* threadgroup leader，线程组的头子 */
	...
	*/
	...
	unsigned long nvcsw, nivcsw; /* context switch counts,上下问切换计数 */
...
/* mm fault and swap info: this can arguably be seen as either mm-specific or thread-specific */
    ...
    /* file system info, 这两个变量是用来防止在do_follow_link中进入死循环，和限制路径名中 符号链接的最大数目 
     * link_count:当前符号链接连续嵌套的次数
     * total_link_count:一个路径中符号链接的总数
	 */
	int link_count, total_link_count;
#ifdef CONFIG_SYSVIPC
/* ipc stuff,ipc相关 */
	struct sysv_sem sysvsem;  // 防止进程获得信号量之后，有进程在等待这个信号量，而当前进程有挂了，用于唤醒等待进程
#endif
/* CPU-specific state of this task,当前进度特定于cpu的状态信息 */
	struct thread_struct thread;
/* filesystem information，文件系统信息,查找文件时用的根目录,当前工作的目录, 和一个可选的根目录(这个似乎用的不多)*/
	struct fs_struct *fs;
...
/* signal handlers,信号处理程序 */
	...
	struct signal_struct *signal; /* 进程信号队列，shared signal queue */
	...
	struct sigpending pending;  /* 线程信号队列, private signal queue */
 ...
 * 3. 如果忽略水印分配仍然失败，那么直接返回ENOMEM，而不会调用OOM killer去杀死进程，释放内存
 ...
 PF_MEMALLOC含义：
 当前进程有很多可以释放的内存，如果能分配一点紧急内存给当前进程，
 那么当前进程可以返回更多的内存给系统。非内存管理子系统不应该使用这个标记，
 除非这次分配保证会释放更大的内存给系统。如果每个子系统都滥用这个标记，
 可能会耗尽内存管理子系统的保留内存。

 */
#define PF_MEMALLOC	0x00000800	/* kswapd进程有这个标记 Allocating memory */
#define PF_FLUSHER	0x00001000	/* 当前进程是pdflush进程 responsible for disk writeback */
