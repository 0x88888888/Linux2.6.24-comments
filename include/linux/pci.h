...
struct module;
/*
 * pci_driver结构用来描述一个PCI设备，
 * 因此所有的PCI驱动都必须创建一个pci_driver结构,
 * 用来描述pci设备管理模块描述PCI驱动程序。
 */
struct pci_driver {
	struct list_head node;
	char *name; //驱动名称，通常被设置为何驱动程序模块相同的名字
	/* 指向pci_device_id结构数组的指针 */
	const struct pci_device_id *id_table;	/* must be non-NULL for probe to be called */
	//当有驱动被添加到内核时,会调用此接口进行设备的初始化
