 ...
 * 放到skb的信息控制块中
 * 用宏IPCB完成类型转换
 * ip层接受数据包的时候，用ip_rcv_options解析ip首部的数据，保存到inet_skb_parm中
 * 在输出时，会用ip_options_build生成ip选项数据
 * 在转发是，用ip_forward_options处理.
 */
...
struct ipcm_cookie
{
    // UDP或者RAW数据报的目的地址，只有当存在IP选项时才设置，用作源路由选项的最后一条地址。
	__be32			addr;
	// UDP或者RAW数据报的输出网络设备
	int			oif;
	//IP数据报的选项信息
