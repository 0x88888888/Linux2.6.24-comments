...
void kobject_init(struct kobject * kobj)
{
	if (!kobj)
		return;
	kref_init(&kobj->kref); /* kobj->kref==1 */
...
int kobject_add(struct kobject * kobj)
{
	...
	if (kobj->kset) {
	    ...
	    /*存在kset，但是parent为空,kobj->kset->kobj做为parent */
		if (!parent)
		...
		spin_unlock(&kobj->kset->list_lock);
		/* kobj->kset->kobj做为parent */
		kobj->parent = parent;
	}

    /* 在 sysfs中 创建kobj对应的目录 */
...
/**
 *  初始化kobj，并且将其添加到父对象中的集合中去(如果有的话),
 *  同时在sysfs文件系统中创建一个对应的项.
...
/**
 *  调用kobj->ktype->release函数，释放分配的资源
