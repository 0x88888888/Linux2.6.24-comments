...
void *radix_tree_lookup(struct radix_tree_root *root, unsigned long index)
{
	...
	height = node->height;
	/*
	 * radix_tree_maxindex()根据高度height,计算出能够映射的最大尺寸的页面号,
	 * 如果index超过这个页面号，就返回NULL
	 */
	if (index > radix_tree_maxindex(height))
		return NULL;

	/* RADIX_TREE_MAP_SHFIT默认为6 */
	shift = (height-1) * RADIX_TREE_MAP_SHIFT;

	do {
		/*
		 * RADIX_TREE_MAP_MASK默认为2^6 -1,也就是0x3F,低6位有效
	     */
		slot = (struct radix_tree_node **)
 ...
 *  查找PAGECACHE_TAG_DIRTY或者PAGECACHE_TAG_WRITEBACK是否有被设置为1的
 *	@root:		radix tree root
 *	@tag:		tag to test
 */
