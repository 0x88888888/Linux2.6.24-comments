...
/*
 * cpuset--->cpuset_subsys_id,
 * debug--->debug_subsys_id,
 * ns--->ns_subsys_id,
 * cpu_cgroup-->cpu_cgroup_subsys_id,
 * cpuacct-->cpu_cpuacct_id
 */
...
struct cgroupfs_root {
	...
	//hierarchy中的顶层cgroup
	// 在每个挂载点下面都会有一个总的cgroup.而通过mkdir创建的cgroup是它的子结点.
...
/*
 * 它先初始化root中的几条链表.因为root中有一个top_cgroup.
 * 因此将root->number_of_cgroups置为1.然后,对root->top_cgroup进行初始化.
 * 使root->top_cgroup.root指向root.
 *
 * cgroup_get_sb()
 *  init_cgroup_root()
 */
static void init_cgroup_root(struct cgroupfs_root *root)
{
	...
	cgrp->root = root;
	/*
	 * root->top_cgroup.top_cgroup指向它的本身.
	 * 因为root->top_cgroup就是目录下的第一个cgroup.
	 */
...
static void cgroup_init_subsys(struct cgroup_subsys *ss)
{
	...
	// 将每个要注册的subsys->root都指向rootnode
	ss->root = &rootnode;
	// 调用subsys->create()生成一个cgroup_subsys_state
	css = ss->create(ss, dummytop);
	/* We don't handle early failures gracefully */
	BUG_ON(IS_ERR(css));
	// 调用init_cgroup_css()将dummytop.subsys[i]设置成ss->create()生成的cgroup_subsys_state
 ...
 *
 * Cgroup的初始化包括两个部份.即cgroup_init_early()和cgroup_init().
 * 分别表示在系统初始时的初始化和系统初始化完成时的初始化.
 * 分为这两个部份是因为有些subsys是要在系统刚启动的时候就必须要初始化的.
 *
 */
