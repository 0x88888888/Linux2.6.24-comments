...
/*
 * 计算request_queue->nr_congestion_off和request_queue->nr_congestion_on的值
 *
...
static void blk_queue_congestion_threshold(struct request_queue *q)
{
	...
	if (nr > q->nr_requests)
		nr = q->nr_requests;
	q->nr_congestion_on = nr; //113

	nr = q->nr_requests - (q->nr_requests / 8) - (q->nr_requests / 16) - 1;
	if (nr < 1)
		nr = 1;
	q->nr_congestion_off = nr; //111
}
...
static int __make_request(struct request_queue *q, struct bio *bio)
{
	...
	switch (el_ret) {
		case ELEVATOR_BACK_MERGE: //向现有的bio向后合并
...
get_rq:
	...
	/* 函数接收一个请求队列描述符的地址q作为其参数。
	   它设置q->queue_flags字段中的QUEUE_FLAG_PLUGGED位；
...
void blk_complete_request(struct request *req)
{
	...
	raise_softirq_irqoff(BLOCK_SOFTIRQ); /* 一个request处理结束了，唤醒相应 的软中断 */
...
int __init blk_dev_init(void)
{
	...
	/*在软中断中调用request_queue->q->softirq_done_fn() */
