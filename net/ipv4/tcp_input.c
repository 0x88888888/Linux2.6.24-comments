...
/*
 * 在发送端有两种RTT的测量方法(具体可见前面blog)，
 * 但是因为TCP流控制是在接收端进行的，所以接收端也需要有测量RTT的方法。
 */
...
static void tcp_rtt_estimator(struct sock *sk, const __u32 mrtt)
{
	...
	if (tp->srtt != 0) {  //不是得到第一个RTT采样
		m -= (tp->srtt >> 3);	/* m is now error in rtt est */
		tp->srtt += m;		/* rtt = 7/8 rtt + 1/8 new, 更新srtt，注意srtt是8倍的rtt值 */
		...
		if (tp->mdev > tp->mdev_max) { // 更新mdev_max 和rttvar
			tp->mdev_max = tp->mdev;
			if (tp->mdev_max > tp->rttvar)
				tp->rttvar = tp->mdev_max;
		}
		
		if (after(tp->snd_una, tp->rtt_seq)) {//过了一个RTT了，更新mdev_max和rttvar
			if (tp->mdev_max < tp->rttvar)
...
static void tcp_mark_head_lost(struct sock *sk, int packets)
{
	...
	tcp_for_write_queue_from(skb, sk) {
		...
		if (!(TCP_SKB_CB(skb)->sacked & (TCPCB_SACKED_ACKED|TCPCB_LOST))) {
			TCP_SKB_CB(skb)->sacked |= TCPCB_LOST; 
			tp->lost_out += tcp_skb_pcount(skb); /* 此段已经被sacked */
...
tcp_fastretrans_alert(struct sock *sk, int pkts_acked, int flag)
{
	...
	case TCP_CA_Loss:
		...
		if (!tcp_try_undo_loss(sk)) {
			tcp_moderate_cwnd(tp); /* 如果不能撤销，则继续重传标志为丢失的包*/
...
static int tcp_process_frto(struct sock *sk, int flag)
{
	...
	if (tp->frto_counter == 1) {
		...
		/*
		 * tcp_frto_response表示TCP在检测到虚假的RTO后，
		 * 采用什么函数来进行阈值和拥塞窗口的调整
		 */
...
static int tcp_ack(struct sock *sk, struct sk_buff *skb, int flag)
{
	...
	if (tcp_ack_is_dubious(sk, flag)) {
		...
		if ((flag & FLAG_DATA_ACKED) && !frto_cwnd)
			tcp_cong_avoid(sk, ack, prior_in_flight, 1); /* 拥塞窗口的调节*/
	}
...
old_ack:
	...
	if (TCP_SKB_CB(skb)->sacked)
		tcp_sacktag_write_queue(sk, skb, prior_snd_una); /* 重新标志各个段的记分牌*/
 ...
 * 
 *
 * 处理处理ESTABLISHED和TIME_WAIT之外的状态
 */
