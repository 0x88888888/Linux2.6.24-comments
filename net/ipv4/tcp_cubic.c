...
/*
 * beta的改变（在BIC中为819）会导致在bictcp_recalc_ssthresh中，
 * 如果启用fast convergence，那么last_max_cwnd = 0.85*snd_cwnd ，
 * 而慢启动阈值=0.7*snd_cwnd 。这样会导致更早的到达平衡值，对snd_cwnd有很大的影响。
 */
...
struct bictcp {
	...
	u32	tcp_cwnd;	/* estimated tcp cwnd, 按照Reno算法计算得的cwnd  */
