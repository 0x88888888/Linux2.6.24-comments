...
int inet_bind(struct socket *sock, struct sockaddr *uaddr, int addr_len)
{
	...
	inet->sport = htons(inet->num); /* 绑定端口 */
	inet->daddr = 0;
	inet->dport = 0;
	sk_dst_reset(sk);
	err = 0;
...
/*
 * 是ip层gso_send_check接口的实现，在分段之前对伪首部进行校验和计算
 */
static int inet_gso_send_check(struct sk_buff *skb)
{
	...
	if (likely(ops && ops->gso_send_check))
		err = ops->gso_send_check(skb); //tcp_v4_gso_send_check
	rcu_read_unlock();
...
static struct sk_buff *inet_gso_segment(struct sk_buff *skb, int features)
{
    ...
    //校验待软GSO分段的SKB，其gsotype是否存在其他非法的值。
    ...
    //待分段的数据报长度至少要大于IP首部的长度，否则必定是无效的IP数据报
	if (unlikely(!pskb_may_pull(skb, sizeof(*iph))))
		goto out;
	...
	skb_reset_transport_header(skb);
	iph = ip_hdr(skb);
	//取出ip首部中的id字段，用于分片数据报的IP首部中的ID的基础值。
	id = ntohs(iph->id);
	proto = iph->protocol & (MAX_INET_PROTOS - 1);
	segs = ERR_PTR(-EPROTONOSUPPORT);

	rcu_read_lock();
	//根据IP首部中的协议字段的值，取出对应的传输层协议接口，然后调用gso_segment接口，对TCP段进行软GSO分段,分段得到的新段通过next链接在原先的段之后
	ops = rcu_dereference(inet_protos[proto]);
	if (likely(ops && ops->gso_segment))
		/*
		 * udp不支持的，
		 * tcp是tcp_tso_segment函数了
		 */
...
static int __init inet_init(void)
{
    ...
    //注册tcp的协议族到proto_list
	rc = proto_register(&tcp_prot, 1);
	if (rc)
		goto out;

    //注册udp的协议族到proto_list
	rc = proto_register(&udp_prot, 1);
	if (rc)
		goto out_unregister_tcp_proto;

    //注册raw原始协议族到proto_list
	 ...
	 * 设置net_families[inet_family_ops->family] = inet_family_ops
	 */
    ...
    /*
     * 分别将TCP,UDP,RAW对应的inet_protosw对象连接到inetsw中去
	 */
