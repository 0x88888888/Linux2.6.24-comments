...
static int ip_local_deliver_finish(struct sk_buff *skb)
{
	...
	resubmit:
		...
		if ((ipprot = rcu_dereference(inet_protos[hash])) != NULL) {
			...
			/* 转到上层协议,tcp_v4_rcv , udp_rcv, icmp_rcv, igmp_rcv */
...
int ip_local_deliver(struct sk_buff *skb)
{
	 ...
	 *  如果接收到的IP数据时分片，则调用ip_defrag()进行重组
	 *  其标志位IP_DEFRAG_LOCAL_DELIVER
	 */

	if (ip_hdr(skb)->frag_off & htons(IP_MF | IP_OFFSET)) {
		/* 重组ip层数据 
		 * 如果ip_defrag返回0，则表示IP数据报分片尚未到期，重组没有完成，
 ...
 *   ip_rcv_options()
 *
 * 当ip层接收到数据报时，先校验该IP数据报有效性，然后在转发或上传到本地上层协议之前，
 * 需要解析该数据报中的IP选项并处理开源路由协议。
 */
...
static int ip_rcv_finish(struct sk_buff *skb)
{
	 ...
	 */
	if (skb->dst == NULL) {
		/* 路由查找,在ip_route_input到ip_route_input_slow的调用中，完成对skb->dst->input的赋值 */
		int err = ip_route_input(skb, iph->daddr, iph->saddr, iph->tos,
					 skb->dev);
		if (unlikely(err)) { //路由查找失败
			if (err == -EHOSTUNREACH)
	...
	/* 这里最后会调用skb->dst->input函数,input函数可以是ip_local_deliver,ip_forward */
...
int ip_rcv(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt, struct net_device *orig_dev)
{
	 ...
	 *
	 * 经过上面的层层检测，到这里已经确定它是ip数据帧了，然后对其进行修剪
	 */
