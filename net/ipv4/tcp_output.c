 ...
 * 如果超时，则会使拥塞窗口失效并重新计算拥塞窗口。然后根据最近接收段的时间，
 * 确定是否进入pingpong模式。
 *
 * 符合三个条件才重置cwnd：
...
static int tcp_tso_should_defer(struct sock *sk, struct sk_buff *skb)
{
	 ...
	 * 一般来说是64KB,这个值在后面的内核版本是sock的成员变量sk_gso_max_size
	 */
	if (limit >= 65536)
		goto send_now;
	...
	if (sysctl_tcp_tso_win_divisor) {
		 ...
		 * just use it.
		 */
		chunk /= sysctl_tcp_tso_win_divisor; /* 单个TSO段可消耗的发送量*/
		if (limit >= chunk)
			goto send_now;
	} else {
