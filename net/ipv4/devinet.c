...
/*
 * inetdev_event() 时NETDEV_REGISTER事件
 *  inetdev_init()
 */
...
/*
 * 根据指定网络设备的网络配置，检查a,b两个ip地址是否属于同一个子网
 */
 ...
 *
 * 在正在匹配的输入设备的主IP地址中查找与前缀和掩码匹配的IP地址，
 * 如果成功，就返回ip地址
 */
...
int devinet_ioctl(unsigned int cmd, void __user *arg)
{
	...
	switch (cmd) {
	case SIOCGIFADDR:	/* 获取指定网络设备的本地网络ip地址, Get interface address */
		sin->sin_addr.s_addr = ifa->ifa_local;
		goto rarok;

	case SIOCGIFBRDADDR:	/* 获取指定网络设备的组播ip地址,Get the broadcast address */
		sin->sin_addr.s_addr = ifa->ifa_broadcast;
		goto rarok;

	case SIOCGIFDSTADDR:	/*在点对点情况下，获取指定网络设备的点对ip地址. Get the destination address */
		sin->sin_addr.s_addr = ifa->ifa_address;
		goto rarok;

	case SIOCGIFNETMASK:	/* 获取子网掩码，Get the netmask for the interface */
	...
	case SIOCSIFADDR:	/*  设置网络设备的本地ip地址, Set interface address (and family) */
		ret = -EINVAL;
	...
	case SIOCSIFBRDADDR:	/* 设置组播地址 Set the broadcast address */
		ret = 0;
	...
	case SIOCSIFDSTADDR:	/*  在点对点的情况下，设置点对点对端的ip地址 Set the destination address */
		ret = 0;
	...
	case SIOCSIFNETMASK: 	/* 设置网络掩码， Set the netmask for the interface */
...
/*
 * 在通过网络设备输出数据时，会调用这个函数，选择source ip地址。
 *
 * icmp_send()
 *  inet_select_addr()
 *
 ...
 *
 * ip_netdev_notifier对象的notifier_call函数
 */
...
void __init devinet_init(void)
{
    ...
    /* 注册netlink接口到rtnl_msg_handlers[PF_INET][].doit =func */	
