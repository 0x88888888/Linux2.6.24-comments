...
static int tcp_write_timeout(struct sock *sk)
{
	...
	if ((1 << sk->sk_state) & (TCPF_SYN_SENT | TCPF_SYN_RECV)) { //TCP三次握手期间
		...
		if (icsk->icsk_retransmits)
			dst_negative_advice(&sk->sk_dst_cache); //之前重传过，那就要更新路由缓存了
			
        // 获取重传次数，对于SYN报文系统默认5次，即内核参数sysctl_tcp_syn_retries，
        // 用户可以通过setsockopt的TCP_SYNCNT选项设置该参数，也可以通过修改/proc里面的内
 ...
 *
 *  超时重传定时器是TCP连接可靠性的重要保证，
 *  其工作原理为TCP连接在发送某一个数据报文或者SYN报文后，
 *  该连接就会启动一个定时器，在规定时间内如果没有收到对端回复的ACK报文，
 *  那么定时器超时处理函数就重新发送数据，直到发送成功或者达到最大重传次数而上报错误为止
 *
 *  重传定时器
 *
 *  tcp_write_timer()
...
static void tcp_retransmit_timer(struct sock *sk)
{
	...
	BUG_TRAP(!tcp_write_queue_empty(sk));
     ...
     /*
      * 收到对端窗口为0，sk状态不为SOCK_DEAD，同时也不处于TCP三次握手期间，
      * 也就是在established状态下出现了拥塞
      *
      */
