...
/* 插入skb到sk->sk_receive_queue，然后通过sk->sk_data_ready函数(通常指向sock_def_readable)唤醒等待进程 
 *
 *
 *  ip_rcv
 *   ip_rcv_finish
...
int sock_queue_rcv_skb(struct sock *sk, struct sk_buff *skb)
{
    ...
    // 应用sk_filter，这允许BPF（Berkeley Packet Filter）过滤器在socket上被应用
 ...
 *      sk_init()
 *
 * 设置接收和发送的缓冲区的大小,SO_RCFBUF,SO_SNDBUF (字节为单位)
 */
 ...
 * 接的状态从TCP_SYN_SENT或TCP_SYN_RECV变为TCP_ESTABLISHED，sock的状态发生变化，
 * sock_def_wakeup()来处理连接状态变化事件，唤醒进程，connect()就能成功返回了。 
 *
 * tcp_v4_rcv
 *  tcp_v4_do_rcv
