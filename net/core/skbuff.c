...
static void kfree_skbmem(struct sk_buff *skb)
{
	...
	case SKB_FCLONE_CLONE: /* 释放的是子skb */
 ...
 *
 * 仅复制skb数据结构，并设置skb数据结构相关成员，clone的skb与原skb共享数据区(data)和skb_shared_info共享(同一份，使用指针引用)，只是增加相应的引用计数而已。
 */
...
struct sk_buff *skb_clone(struct sk_buff *skb, gfp_t gfp_mask)
{
	...
	if (skb->fclone == SKB_FCLONE_ORIG &&
	    n->fclone == SKB_FCLONE_UNAVAILABLE) { /* skb是从skbuff_fclone_cache中分配的，所以skb下一个已经是备用了 */
	    /* 只增加引用 */
 ...
 *
 *  复制skb描述符，同时也复制skb的数据缓存区(skb_shared_info)
 */
		     ...
		     gfp_t gfp_mask)
{
	...
	for (i = 0; i < skb_shinfo(skb)->nr_frags; i++)
		get_page(skb_shinfo(skb)->frags[i].page);//引用加 1

	if (skb_shinfo(skb)->frag_list)
		skb_clone_fraglist(skb); /* frag_list中的每个sk_buff引用都加1 */
...
unsigned char *__pskb_pull_tail(struct sk_buff *skb, int delta)
{
	 ...
	 * 
	 * 如果执行goto，这当然是最好的了，skb_shinfo(skb)->frag_list如果是NULL，
	 * 则表明上面的skb_copy_bits最坏只是从skb_shinfo(skb)->frags[]当中拷贝了数据。
	...
	if (eat) {
		...
		/*
		 * 上面这些被free掉的skb，其包含的数据全部是重复的，所以可以完全释放掉。
		 * 但是insp指向的sk_buff可能包含部分重复的数据，这个也需要进行处理，
		 * 请看上一个代码块中if-else中else语句里的处理过程。
		 */
 ...
 *
 * 拆分skb中的数据到skb1中去，
 * 需要注意是否还需拆skb_shared_info中的数据
 */
void skb_split(struct sk_buff *skb, struct sk_buff *skb1, const u32 len)
{
	int pos = skb_headlen(skb);

	if (len < pos)	/* Split line is inside header. 只拆分skb->data指向的数据*/
...
void __init skb_init(void)
{
	...
	/*
	 * 如果在分配SKB对象时，就知道SKB对象会被复制，就应该从这个缓存中分配
	*/
