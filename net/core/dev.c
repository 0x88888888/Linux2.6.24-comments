...
void dev_add_pack(struct packet_type *pt)
{
	...
	if (pt->type == htons(ETH_P_ALL))
		/* 在内核中没有主动加入到ptype_all中的packet_type,
		 * 用户态以socket(PF_PACKET,SOCK_RAW,htons(ETH_P_ALL)) 这种创建的东西会不会加入到此?
		 */
		list_add_rcu(&pt->list, &ptype_all);
	else {
		/*
		 * ax25_packet_type, dn_dix_packet_type, econet_packet_type,
		 * ip_packet_type, ipx_dix_packet_type, ipx_8023_packet_type,
		 * x25_packet_type, aoe_pt,arp_packet_type,vlan_packet_type
		 * sppp_packet_type, pppoed_ptype, pppoes_ptype
		 * llc_tr_packet_type, llc_packet_type,lapbeth_packet_type
		 * ipv6_packet_type,bootp_packet_type, rarp_packet_type
		 * 
	     */
...
int dev_close(struct net_device *dev)
{
	 ...
	 */
	if (dev->stop)
		dev->stop(dev); /* cs89x0.c中的net_close */
...
static void dev_queue_xmit_nit(struct sk_buff *skb, struct net_device *dev)
{
	...
	rcu_read_lock();
	/* 通过socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL))创建的在ptype_all中 */
...
/*
 * skb已经被处理完成，将skb放入softnet_data->completion_queue中，
 * 在软中断处理中释放skb
 */
...
struct sk_buff *skb_gso_segment(struct sk_buff *skb, int features)
{
	...
	if (WARN_ON(skb->ip_summed != CHECKSUM_PARTIAL)) {
		//如果带分割的skb是克隆的，则需要重新分配SKB的线性数据区
	...
	list_for_each_entry_rcu(ptype, &ptype_base[ntohs(type) & 15], list) {
		if (ptype->type == type && !ptype->dev && ptype->gso_segment) {
			...
			/*
			 * 软件处理gso数据包
			 * 如果是ETH_P_IP的packet_type,处理函数是inet_gso_segment
			 */
...
int dev_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	if (likely(!skb->next)) { //不是skb链表了
		...
		if (netif_needs_gso(dev, skb)) { //是GSO数据包，但是网络设备不支持这种特性
			...
			if (unlikely(dev_gso_segment(skb))) //就调用dev_gso_segment对GSO数据包进行软件分割
				goto out_kfree_skb;
			
			if (skb->next) //分割后变成多个skb对象了
				goto gso;
		}
 
        /* 指向cs89x0.c中的net_send_packet
         * 或者e100的e100_xmit_frame
         */
...
int dev_queue_xmit(struct sk_buff *skb)
{
	 ...
	 * 如果是gso数据包，并且网络设备支持GSO数据包的处理，
	 * 则跳转到gso标号处对GSO数据包直接处理。
	 */
	if (netif_needs_gso(dev, skb))
		goto gso;

    /*
     * 首先判断skb是否被分段,如果分了段并且网卡不支持分散读的话需要将所有的段重新合并成一个段
     * 这里的__skb_linearize起始就是__pskb_pull_tail(skb, skb->data_len),这个函数基本上等同于pskb_may_pull
     * pskb_may_pull的作用就是检测skb对应的主buf中是否有足够的空间来pull处len长度,
     * 如果不够就重新分配skb并将frags中的数据拷贝入新分配的主buff中,而这里将参数len设置为skb->datalen,
     * 也就是会将所有的数据全部拷贝到buff中，以这种方式完成skb的线性化
     *
     * 是FRAGLIST类型的聚合分散I/O数据包，但是网络设备又不支持FRAGLIST类型的聚合分散I/O数据包,
     * 就调用__skb_linearize，对FRAGLIST数据包进行线性化，如果失败，就失败了。
     */
...
gso:
	 ...
	 */
	 //到此说明设备没有队列了，就需要调用dev_hard_start_xmit()直接发送了
	if (dev->flags & IFF_UP) {
 ...
 *
 *	return values:
 *	NET_RX_SUCCESS	(no congestion)，没有拥塞
 *	NET_RX_DROP     (packet was dropped)，包被丢弃了
 *
 *  网络接受数据程序时调用
 *
...
int netif_rx(struct sk_buff *skb)
{
	 ...
	 * 先传给netpoll模块，如果netpoll模块处理了，就不往下传了
	 */
	if (netpoll_rx(skb))
		return NET_RX_DROP;

    /* 记录数据包的接收时间skb->tstamp */
	if (!skb->tstamp.tv64)
		net_timestamp(skb);
	...
	local_irq_save(flags);
	// 得到处理器的队列
	queue = &__get_cpu_var(softnet_data); /* 得到当前cpu专门放网络数据的队列对象 */
...
static void net_tx_action(struct softirq_action *h)
{
    ...
    /*
     * 先释放掉已经被传输完成的skb对象
     * 由dev_kfree_skb_irq()放入softnet_data->completion_queue中
	 */
...
int netif_receive_skb(struct sk_buff *skb)
{
	 ...
	 * 是经过NAPI进来的数据，则在此需将数据包转给netpoll模块，
	 * 如果有netpoll接收了，就不需要再传递给协议栈处理了
	 */
...
ncls:
	...
	type = skb->protocol; /* 对应的L3层的协议,IP,ARP... */
...
/*
 * 在net_dev_init中设置 &per_cpu(softnet_data)->backlog.poll=process_backlog
 *
 * irq_exit()
 *  do_softirq()
 *   __do_softirq()
...
{
    /* 放在本cpu上的网络数据包的等待队列(napi_struct对象) */
	struct list_head *list = &__get_cpu_var(softnet_data).poll_list;
	unsigned long start_time = jiffies;
	
	int budget = netdev_budget;//默认是300
	void *have;
	...
	while (!list_empty(list)) {
		 ...
		 *
		 * 控制循环次数，循环执行的时间
		 * 从网卡读取的数据总不能无限大吧，从网卡读数据的时间不能无限长吧，需要控制一下
		 */
        ...
        /* 设置napi_struct->poll_owner=cpuid */
		have = netpoll_poll_lock(n);

		weight = n->weight; // weight一般是64
		 ...
		 *
		 * 在netif_napi_add中设置NAPI_STATE_SCHED
		 */
		work = 0;
		 ...
		 *
		 * 读取的数据已经到达配额了,将设备挂到softnet_data->poll_list队列尾部去
		 */
		if (unlikely(work == weight)) {
			if (unlikely(napi_disable_pending(n)))
				__napi_complete(n); //设备从poll_list中移除
			else
...
static int dev_new_index(struct net *net)
{
	...
	for (;;) {
		if (++ifindex <= 0)
			ifindex = 1;
		if (!__dev_get_by_index(net, ifindex)) /* ifindex未被分配，就返回去 */
			return ifindex;
	}
}
...
/*
 * 包含以注销即将结束的网络设备。 
 * 在调用unregister_netdevice注销设备后，会调用net_set_todo 将注销的设备连接到net_todo_list上去
 * 然后rtnl_unlock会释放锁，并调用netdev_run_todo完成对网络设备的注销操作.
 */
...
static int __init net_dev_init(void)
{
    ...
    /* 注册 /proc/net/dev   和/proc/net/softnet_stat文件 */
	if (dev_proc_init())
		goto out;
	...
	/* 通过变量 ptype_all为ETH_P_ALL类型数据包初始化上层协议处理方法 */
	INIT_LIST_HEAD(&ptype_all);
	/* 通过变量 ptype_base注册协议的数据包初始化上层协议处理方法 */
	for (i = 0; i < 16; i++)
