 ...
 *   __neigh_event_send()
 *
 * 如果需要ARP探测，ARP将立即被发送。__neigh_event_send将返回0，表示邻居被视为“已 连接”或“已延迟”，否则返回1
 */
...
int neigh_resolve_output(struct sk_buff *skb)
{
    ...
    // 返回0，表示邻居被视为“已 连接”或“已延迟”
	if (!neigh_event_send(neigh, skb)) {
		
		int err;
		struct net_device *dev = neigh->dev;
	   /*
	    * 如果邻居关联的设备的协议实现支持缓存硬件头，并且当前没 有缓存，
	    * neigh_hh_init将缓存它。
	    */
...
#endif	/* CONFIG_SYSCTL */
/*
 * 邻居子系统初始化函数，在启动时的initcall阶段调用
 */
static int __init neigh_init(void)
{
    //向netlink子系统，注册各种处理函数
