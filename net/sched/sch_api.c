...
dev_graft_qdisc(struct net_device *dev, struct Qdisc *qdisc)
{
	...
	qdisc_lock_tree(dev);
	if (qdisc && qdisc->flags&TCQ_F_INGRESS) { //如果安装的是输入排队规则
 ...
 * 该函数检查是否接受skb->protocol所指定的协议，然后调用classify做出决定是否接受或分到哪个类中
 */
...
{
    /* 将pfifo_qdisc_ops, bfifo_qdisc_ops挂到qdisc_base链表上去 */
