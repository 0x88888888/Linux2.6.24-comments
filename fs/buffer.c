 ...
 *
 * 将内存页上的数据写到磁盘上完成时调用，唤醒等待在该页上的数据
 *
 */
static void end_buffer_async_write(struct buffer_head *bh, int uptodate)
{
	...
	end_page_writeback(page);/*唤醒等待在该页上的进程*/
	return;
 ...
 *
 *
 * 该函数将address_space->private_list中所有的私有的inode缓存区写回，
 * 这些通常都是用于保存间接块或者其他文件系统内部数据，他们不是inode管理数据的一部分，而用于管理数据自身
 */
			...
			unsigned long blocksize, unsigned long b_state)
{
	...
	do {
		bh->b_state |= b_state;
		tail = bh;
		bh = bh->b_this_page; /*下一个 */
	} while (bh);
	tail->b_this_page = head; /* 首尾连好 */

	spin_lock(&page->mapping->private_lock);

	/* 根据page的状态设置buffer_head的状态 */
	...
	attach_page_buffers(page, head);/* 设置private标志位，设置page->private=head */
			...
			get_block_t *get_block, struct writeback_control *wbc)
{
	...
	if (!page_has_buffers(page)) { /* 检查page->flags & PG_private */

	    /* 如果page没有建立相应的buffer_head，则建立相应的buffer_head对象 */
	...
	head = page_buffers(page); /*page->private 就是该page对应的buffer_head链表*/
	bh = head;

	/*
	 * Get all the dirty buffers mapped to disk addresses and
	 * handle any aliases from the underlying blockdev's mapping.
	 * 第一次遍历，得到素有dirty buffer在磁盘上的块号
	 */
	do {
		if (block > last_block) {
			/* 操作inode中能够处理的block号码大小了
			...
			/* ext2_get_block，查找块设备上与bh相匹配的blcok */
    ...
    /* 第二次遍历，在写到磁盘上去之前，做一些buffer_head上的标记 */
	do {
		if (!buffer_mapped(bh))
			continue;
		/*
		...
		if (test_clear_buffer_dirty(bh)) {
			/* 设置buffer_head->b_end_io=end_buffer_async_write,会在该函数中唤醒等待在该页上的进程 */
	...
	set_page_writeback(page);/* 设置page->flag |=PG_writeback */

	do {
		struct buffer_head *next = bh->b_this_page;
		if (buffer_async_write(bh)) {
			submit_bh(WRITE, bh); /* 提交到BIO层 */
			nr_underway++;
		}
		bh = next;
	} while (bh != head);
...
int block_read_full_page(struct page *page, get_block_t *get_block)
{
	...
	do {
		...
		arr[nr++] = bh; /* 如果buffer_head已经映射到page的部分了，但是内容不是最新的，就加入到这个数组里面去了 */
	...
	if (!nr) {
		/* 说明所有buffer_head负责的数据都是uptodate了，也就是整个page都是uptodate了
    ...
    /* 进入第二阶段，说明page中有数据不是最新的了 */
	/* Stage two: lock the buffers */
	for (i = 0; i < nr; i++) {
		bh = arr[i];
		lock_buffer(bh);
		mark_buffer_async_read(bh); /* 只是设置bh0>b_end_io=end_buffer_async_read*/
	}
