...
/* 调整group->bg_free_blocks_count和super_block */
	...
	while (1) {
		...
		if (rsv_is_empty(&my_rsv->rsv_window) /* 检查是否有预留区域 */|| (ret < 0) ||
			...
			if (!goal_in_my_reservation(&my_rsv->rsv_window,
							grp_goal, group, sb))
				grp_goal = -1;//依旧不在reservation window中
		...
		/*
		 * 本while中，前面的都是调整工作 
		 * 如果预留窗口中可以分配，就从预留窗口中分配，否则就从grp_goal中分配 
		 */
		    ...
		    unsigned long *count, int *errp)
{
	...
	*errp = -ENOSPC;
	...
	sbi = EXT2_SB(sb); /* ext2_sb_info */
	es = EXT2_SB(sb)->s_es; /* ext2_super_block */
	ext2_debug("goal=%lu.\n", goal);
	/*
	...
	if (block_i) {
		...
		if (windowsz > 0) /* 可以有预分配 */
			my_rsv = &block_i->rsv_window_node;
	}

	/* 判断是否有free的block ，根据sbi->s_freeblocks_counter判断 */
	...
	if (goal < le32_to_cpu(es->s_first_data_block) ||
		...
		goal = le32_to_cpu(es->s_first_data_block); /* 不在有效范围，调整一下 */
...
retry_alloc:
	...
	/*
	 * 可以用预留空间，但是free的block又不够预留条件，那就关闭预留
