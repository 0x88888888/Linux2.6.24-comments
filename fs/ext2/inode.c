			   ...
			   int create /* 是否创建block */)
{
	...
	if (depth == 0)
		return (err);
reread:
    /*  根据offsets[]数组中的数据，跟踪到相应的磁盘block */
...
{
    /* 从vfs层的inode获取到ext2_inode_info结构 */
	struct ext2_inode_info *ei = EXT2_I(inode);
	ino_t ino = inode->i_ino;
	struct buffer_head * bh;
	/*
	 * 请求磁盘驱动程序读取inode结构，保存在raw_inode中
	 */
    ...
    /*
     * 根据raw_inode设置vfs inode和ext2内存inode
	 */
	...
	if (S_ISREG(inode->i_mode)) { /* 普通文件 */
		...
		if (ext2_inode_is_fast_symlink(inode)) /* 路径名称<60字符 */
