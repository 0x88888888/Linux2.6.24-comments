		...
		int (*check_acl)(struct inode *inode, int mask))
{
	...
	if (current->fsuid == inode->i_uid)
		...
		if (in_group_p(inode->i_gid)) /* 进程的组信息中，是否有inode->i_gid */
			mode >>= 3;//组权限
	}
...
static struct dentry * real_lookup(struct dentry * parent, struct qstr * name, struct nameidata *nd)
{
	 ...
	 */
	result = d_lookup(parent, name); /* 首先在dentry_hashtable中查找 */
	if (!result) {
...
{
	dput(nd->dentry);
	/* 如果子目录项对应一个新的mount point,那么需要改变nd->mnt */
	if (nd->mnt != path->mnt)
		mntput(nd->mnt);
...
static __always_inline int __do_follow_link(struct path *path, struct nameidata *nd)
{
	...
	mntget(path->mnt);
	/*
     * ext2: ext2_fast_symlink_inode_operations->ext2_follow_link()  
     *       ext2_symlink_inode_operations->page_follow_link_light() 
     *
	 */
		     ...
		     struct path *path)
{
	...
	path->mnt = mnt;
	path->dentry = dentry;
	__follow_mount(path); /* 如果是挂载点，切换到挂载文件系统的根目录 */
	return 0;

need_lookup:
	dentry = real_lookup(nd->dentry, name, nd); /* 去文件系统查找 */
	if (IS_ERR(dentry))
		goto fail;
	goto done;
...
static fastcall int __link_path_walk(const char * name, struct nameidata *nd)
{
	...
	/* 跳过 / 之后得到的是一个空字符串,直接返回 */
	if (!*name)
		goto return_reval;

    /* 当前起点的dentry对象对应的inode */
	inode = nd->dentry->d_inode;
	/* 如果depth不为0，设置LOOKUP_FOLLOW标记 */
	if (nd->depth)
		lookup_flags = LOOKUP_FOLLOW | (nd->flags & LOOKUP_CONTINUE);

	/* At this point we know we have a real path component. 
     * 下面这个循环就是根据路径中的/，一级级的分离出目录名，
     * 然后处理。
	*/
	for(;;) {
		unsigned long hash;
		 ...
		 * 在nd的dentry成员中搜索由this指定的的子目录项节点，
		 * 并在next中返回子目录项节点的dentry.
		 */
		...
		if (inode->i_op->follow_link) {
			...
			if (!inode->i_op)
				break;
		} else /* 不是symbol   link,nd中的dentry成员就可以前进一步了*/
