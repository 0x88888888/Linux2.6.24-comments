...
/* 打开 /sysfs中的文件时，file->private关联到sysfs_buffer */
struct sysfs_buffer {
...
static int fill_read_buffer(struct dentry * dentry, struct sysfs_buffer * buffer)
{
	...
	/* 这个buffer->ops在sysfs_open_file中设置 */
...
/**
 *	sysfs_read_file - read an attribute.  sysfs中读取文件(即kobject的某个attributes)
 *	@file:	file pointer.
 *	@buf:	buffer to fill.
...
sysfs_read_file(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (buffer->needs_read_fill) { /* 缓冲区中没有数据，需要填充 */
