 ...
 * directory entry.
 * 从bootmem alloctor分配一个页面，并且把页面的物理地址，填入页目录项中
 *
 * start_kernel()
 *  setup_arch()
 *   paging_init()
...
static void __init kernel_physical_mapping_init(pgd_t *pgd_base)
{
    ...
    /* PTRS_PER_PGD表示页目录表中有多少页，这里为1024 */
	for (; pgd_idx < PTRS_PER_PGD; pgd++, pgd_idx++) {
		/* 在x86,32位系统上，返回pgd本身 */
		pmd = one_md_table_init(pgd);
		/* 保证只映射max_low_pfn个页面(896M的内存) */
		if (pfn >= max_low_pfn)
			continue;
 ...
 * 用于高端内存内核映射的第一个固定映射内存区的地址保存在全局变量kmap_vstart中
 *
 *
 * start_kernel()
 *  setup_arch()
...
/*
 * 用于停用bootmem分配器,启用buddy allocator,并迁移到实际的内存管理函数 
 * start_kernel()
 *  mem_init()
 *
 */
 ...
 *     free_initmem()
 *
 * 释放掉各种__init,__init_data,__setup_param,各种xxx_initcall标记的内存，即
 */
