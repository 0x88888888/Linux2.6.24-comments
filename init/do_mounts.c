 ...
 *
 *  通过解析出来的命令行参数” root=/dev/abcdef”来挂接根文件系统 mount_root()
 *
 * start_kernel()
 *  rest_init() 中调用kernel_thread()创建kernel_init线程
 *   prepare_namespace()
...
void __init prepare_namespace(void)
{
	...
	if (saved_root_name[0]) {
		...
		if (!strncmp(root_device_name, "mtd", 3)) {/* 是mtd开头 */
			/* 如果是mtd开头的，直接挂载，然后到返回处 */
